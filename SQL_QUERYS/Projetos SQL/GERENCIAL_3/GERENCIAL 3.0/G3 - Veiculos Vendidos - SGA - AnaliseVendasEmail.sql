...
AND     NotaFiscal.NotaFiscal_DataEmissao                   >= '2021-05-01 00:00:00'
AND     NotaFiscal.NotaFiscal_DataEmissao                   <= '2021-05-18 23:59:59'
AND     Estoque.Estoque_Sigla                               IN ('VN','SL')
					    ...
					    Empresa.Empresa_Codigo = COALESCE( -- Veiculos usados Faturados pela Peugeot RJ, atribui a venda a unidade Fiat Uruguai
					    	                              ...
					    	                              (select top 1 sga_comercialVendedorEmpresa.emp_cd
														   ...
														   (select top 1 sga_empresas.emp_cd
															...
															--AND  ger_nfs.nf_dtemis <= r038hfi.datalt
															AND  (r038hfi.tipadm IN ('3','4')) 
															group by sga_empresas.emp_cd, r034fun.datafa
															--having (MAX(r034fun.datafa) <= ger_nfs.nf_dtemis or r034fun.datafa is null)
															order by r034fun.datafa desc ),
															(SELECT top 1 sga_empresas.emp_cd
															FROM gama..r034fun 
															JOIN gama..r038hfi (NOLOCK) ON r038hfi.numcad = r034fun.numcad 
...
SELECT codigoEmpresa, veiculosVendidos	= SUM(vendido + devolucaoAnterior)
FROM #tmpVendas
