...
//        $destino = json_decode($editData->destino);
        ...
        $percentuais    = DB::select("  SELECT  Percentual.idTabela,
        ...
        foreach ($percentuais as $row => $data) {
            $hideValue[$data->idCentroCusto] = $data->percentual;
        }
...
<div class="container">
    ...
    <!-- Cabeçalho -->
    <div class="row">
        <div class="col-5">CENTRO DE CUSTO</div>
        <div class="col-5">PERCENTUAL</div>
        <div class="col-2 pl-0">INCLUIR / ALTERAR</div>
    </div>
    ...
    <div id="listaCC" class="pt-3">
        ...
        @forelse ($percentuais as $row => $data)
            <div class="row border-bottom border-light pt-2" id="PERC_{{$data->idCentroCusto}}">
                <div class="col-5">{{$data->idCentroCusto}} - {{$data->descricaoCentroCusto}}</div>
                <div class="col-5">{{number_format($data->percentual,2,',','.')}}%</div>
                <div class="col-2">
                    <button type="button" class="btn btn-danger btn-sm" onClick="delPercent('{{$data->idCentroCusto}}')"> <span class="fa fa-trash fa-lg"></span> </button>
                </div>
            </div>
        @empty
